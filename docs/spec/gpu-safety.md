...
|---|---|---|
| `maxabs(Y_gpu − Y_cpu)` | **≤ 0.05** при `rms(x) ≈ 1` | [stitch-gpu.md](stitch-gpu.md), token-loop §7.3 этаж 1 |
| `rmse` | репортить, гейта нет | ловит «плохо везде» против «плохо в одном элементе» |

`maxabs > 0.05` — **баг ядра**, а не квантования: оракул декодирует те же байты. Смотреть в первую очередь: маска `k >= K` на хвосте, шкала группы `k/64`, порядок нибблов, гонка стейджей `cp.async` на `K >= 512`.
