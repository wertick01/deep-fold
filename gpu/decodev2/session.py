...
__all__ = ["DecodeV2Loop", "decodev2_refused", "pick_executor"]
...
def decodev2_refused(report, plan=None) -> str | None:
...
def pick_executor(requested: str, report, plan=None) -> tuple[str, str | None]:
    ...
    why = decodev2_refused(report, plan)
    if want == "tokenloop":
...
class DecodeV2Loop:
    ...
    def weight_bytes(self) -> int:
        """Device bytes of packed NF4 + dense embed, deduplicated by pointer."""
        seen: dict[int, int] = {}
    ...
    def warmup(self, *, prompt: int = 8, tokens: int = 8) -> float:
        ...
        self.state.token.copy_(self.state.next_token)
        for _ in range(max(0, min(int(tokens), 4))):
